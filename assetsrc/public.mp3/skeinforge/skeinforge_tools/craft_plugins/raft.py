...
class RaftRepository:
	...
	def __init__( self ):
		...
		self.base2InfillDensity = settings.FloatSpin().getFromValue( 0.3, 'Interface Infill Density (ratio):', self, 0.9, 0.5 )
		...
		self.interfaceInfillDensity = settings.FloatSpin().getFromValue( 0.3, 'Support Infill Density (ratio):', self, 0.9, 0.5 )
...
class RaftSkein:
	...
	def __init__( self ):
		...
		self.version=settings.getfirmware()
	...
	def addBaseLayer( self ):
		...
		self.raftclass = RaftRepository()
		self.raftclass = settings.getReadRepository( self.raftclass )
		if self.raftclass.baseLayers.value == 1:
			...
			self.BfBaddLayerFromSegments( feedRateMultiplier, self.repository.baseFlowRateMultiplier.value, baseLayerThickness, self.baseLayerThicknessOverLayerThickness, self.baseSegments, z )
	...
	def addBaseSegments( self, baseExtrusionWidth, baseStep ):
		...
		interfaceSegmentsTableKeys = self.interfaceSegmentsTable2.keys()
		interfaceSegmentsTableKeys.sort()
		baseYTableTable = {}
		for interfaceSegmentsTableKey in interfaceSegmentsTableKeys:
			interfaceSegments = self.interfaceSegmentsTable2[ interfaceSegmentsTableKey ]
		...
		for baseYTableTableKey in baseYTableTableKeys:
			...
			for baseYTableKey in baseYTableKeys:
				y = baseYTableKey * self.baseStep2
	...
	def addInterfaceLayer( self ):
		...
		self.interfaceIntersectionsTableKeys2.sort()
		if len( self.interfaceSegments2 ) < 1:
		...
		self.addLayerFromSegments( feedRateMultiplier, flowRateMultiplier, interfaceLayerThickness, self.interfaceLayerThicknessOverLayerThickness, self.interfaceSegments2, z )

	def addInterfaceTables( self, baseStep, interfaceExtrusionWidth ):
		"Add a interface tables. for support"
		interfaceOverhang = self.repository.infillOverhangOverExtrusionWidth.value * interfaceExtrusionWidth
		self.interfaceSegments = []
		self.interfaceSegments2 = []
		self.interfaceIntersectionsTableKeys = self.interfaceIntersectionsTable.keys()
		self.interfaceSegmentsTable = {}
		self.interfaceIntersectionsTableKeys2 = self.interfaceIntersectionsTable2.keys()
		self.interfaceSegmentsTable2 = {}
		...
		"BFB CODE Add a interface tables for interface and raft"
		for yKey in self.interfaceIntersectionsTableKeys2:
			self.interfaceIntersectionsTable2[ yKey ].sort()
			y = yKey * self.baseStep2
			lineSegments = euclidean.getSegmentsFromXIntersections( self.interfaceIntersectionsTable2[ yKey ], y )
			xIntersectionIndexList = []
			for lineSegmentIndex in xrange( len( lineSegments ) ):
				lineSegment = lineSegments[ lineSegmentIndex ]
				endpointBegin = lineSegment[ 0 ]
			...
			joinedSegments = euclidean.getSegmentsFromXIntersections( xIntersections, y )
			if len( joinedSegments ) > 0:
				self.interfaceSegmentsTable2[ yKey ] = joinedSegments
			self.interfaceSegments2 += joinedSegments

	def BfBaddLayerFromSegments( self, feedRateMultiplier, flowRateMultiplier, layerLayerThickness, layerThicknessRatio, segments, z ):
		"BFB CODE Add a layer from segments and raise the extrusion top."
		...
		self.raftclass = RaftRepository()
		self.raftclass = settings.getReadRepository( self.raftclass )
		self.carveclass = carve.CarveRepository()
		self.carveclass = settings.getReadRepository( self.carveclass )
		...
		if self.bfbraft == False:
			for path in paths:
				simplifiedPath = euclidean.getSimplifiedPath( path, layerLayerThickness )
				self.distanceFeedRate.addGcodeFromFeedRateThreadZ( feedRateMinute, travelfeedRateMinute, simplifiedPath, z )
				self.distanceFeedRate.addGcodeFromFeedRateThreadZ( feedRateMinute, travelfeedRateMinute, simplifiedPath, z )
				
		elif self.bfbraft == True:
			for path in paths:
				simplifiedPath = euclidean.getSimplifiedPath( path, layerLayerThickness )
				"First 3 DRAFT DOTS----------------------------------------------------------------------------------------------------------------------"
				self.myx = simplifiedPath[0].real
				self.myx =self.myx-5
				self.myy = simplifiedPath[0].imag
				self.mypath[0]=complex(self.myx,self.myy)
				self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute, travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
				self.myy = self.myy-5
				self.mypath[0]=complex(self.myx,self.myy)
				self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute, travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
				self.myy = self.myy-5
				...
				self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute, travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
				"------------------------------------------------------------------------------------------------------------------------------"
				for p in range(0,len(simplifiedPath)-1):
					self.myx = simplifiedPath[p].real
					self.myy = simplifiedPath[p].imag
					self.myx2 = simplifiedPath[p+1].real
					self.myy2 = simplifiedPath[p+1].imag
					"check amount of dashes"
					self.cntx = abs((self.myx-self.myx2)/10) 
					self.cnty = abs((self.myy-self.myy2)/10)
					
					if self.cntx>self.cnty:
						self.strokes=round(self.cntx)
					else:
						self.strokes=round(self.cnty)
					"Make many dashes"
					if self.strokes>1:
						for i in range(0,self.strokes):
							
							self.mypath[0]=complex(self.myx,self.myy)
							"1st point of 2nd stroke"
							if self.myx!=self.myx2:
								if self.myx<self.myx2:
									self.myx=self.myx+10
								else:
									self.myx=self.myx-10
							if self.myy!=self.myy2:
								if self.myy<self.myy2:
									self.myy=self.myy+10
								else:
									self.myy=self.myy-10
							self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute*2,travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
					else:
						"One dash"
						self.mypath[0]=complex(self.myx,self.myy)
						self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute*2,travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
				"Last DOT----------------------------------------------------------------------------------------------------------------------"
				self.myx = simplifiedPath[-1].real
				self.myy = simplifiedPath[-1].imag
				self.mypath[0]=complex(self.myx,self.myy)
				self.distanceFeedRate.BfBaddGcodeFromFeedRateThreadZ( feedRateMinute*2,travelfeedRateMinute, self.mypath, z, self.carveclass.layerThickness.value )
				"------------------------------------------------------------------------------------------------------------------------------"
	...
	def addRaft( self ):
		...
		if self.raftclass.baseLayers.value == 1:
			...
			baseStep = 9.65
		self.baseStep2 = baseExtrusionWidth / self.repository.base2InfillDensity.value
		...
		self.interfaceIntersectionsTable2 = {}
		euclidean.addXIntersectionsFromLoopsForTable( outsetSeparateLoops, self.interfaceIntersectionsTable, self.interfaceStep )
		euclidean.addXIntersectionsFromLoopsForTable( outsetSeparateLoops, self.interfaceIntersectionsTable2, self.baseStep2 )
		
		if len( self.supportLayers ) > 0:
			supportIntersectionsTable = self.supportLayers[ 0 ].xIntersectionsTable
			supportIntersectionsTable2 = self.supportLayers[ 0 ].xIntersectionsTable2
			
			euclidean.joinXIntersectionsTables( supportIntersectionsTable, self.interfaceIntersectionsTable )
			euclidean.joinXIntersectionsTables( supportIntersectionsTable2, self.interfaceIntersectionsTable2 )
		...
		if self.repository.baseLayers.value > 0 or self.repository.interfaceLayers.value > 0:
			...
			if self.layerStarted:
				self.distanceFeedRate.addLine( '(</layer>)' )
				self.layerStarted = False
				"BFB CODE Raft is finished"
				self.distanceFeedRate.LS = False
			self.distanceFeedRate.addLine( '(<raftLayerEnd> </raftLayerEnd>)' )
			"BFB CODE correct temperature after raft"
			self.endp3 = self.getSupportEndpoints3()
			self.raftclass = RaftRepository()
			self.raftclass = settings.getReadRepository( self.raftclass )
			self.bfbclass = bfb.BfbRepository()
			self.bfbclass = settings.getReadRepository( self.bfbclass )
			...
			"if not use support"
			if self.raftclass.supportChoiceNone.value == True or self.endp3==[] :
				"Raft is finished used Extruder 2"
				if self.bfbclass.useRaft.value==True:
					if self.version == "x1":
						self.start = 1
						self.distanceFeedRate.addLine( 'M204 S'+str(self.bfbclass.secondStandbyT.value) )
						...
						self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.firstLayerT.value) )
					...
					"Raft is finished used Extruder 1"
					self.distanceFeedRate.addLine( 'M107' )
					self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.firstLayerT.value) )
					if self.bfbclass.UseFanEx1.value== True:
						"Turn ON FAN"
				...
				"if use support and use 2 extruder"
				if self.bfbclass.useSupport.value == True:
					self.distanceFeedRate.addLine( 'M107' )
					self.distanceFeedRate.addLine( 'M204 S'+str(self.bfbclass.SupportT.value))
					if self.bfbclass.UseFanEx2.value== True:
						self.distanceFeedRate.addLine( 'M106')
					else:
						self.distanceFeedRate.addLine( 'M107')
				else:
					"if use support and use 1 extruder"
					self.distanceFeedRate.addLine( 'M107' )
					self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.SupportT.value))
					if self.bfbclass.UseFanEx1.value== True:
						"Turn ON FAN"
						self.distanceFeedRate.addLine( 'M106')
					else:
			...
			"BFB CODE Layer is Finished"
			self.distanceFeedRate.LS2 = False
	...
	def addSupportSegmentTable( self, layerIndex ):
		...
		numberOfSubSteps = 4
		"BFB CODE Get intersection for Support"
		...
		"BFB CODE Get intersection for Interface and Raft"
		subStepSize = self.baseStep2 / float( numberOfSubSteps )
		subStepSize2 = self.baseStep2/1.3 / float( numberOfSubSteps )
		aboveIntersectionsTable = {}
		euclidean.addXIntersectionsFromLoopsForTable( aboveLoops, aboveIntersectionsTable, subStepSize )
		outsetIntersectionsTable = {}
		euclidean.addXIntersectionsFromLoopsForTable( outsetSupportLoops, outsetIntersectionsTable, subStepSize2 )
		euclidean.subtractXIntersectionsTable( aboveIntersectionsTable, outsetIntersectionsTable )
		for aboveIntersectionsTableKey in aboveIntersectionsTable.keys():
			supportIntersectionsTableKey = int( round( float( aboveIntersectionsTableKey ) / numberOfSubSteps ) )
			xIntersectionIndexList = []
			if supportIntersectionsTableKey in supportLayer.xIntersectionsTable2:
				euclidean.addXIntersectionIndexesFromXIntersections( 0, xIntersectionIndexList, supportLayer.xIntersectionsTable2[ supportIntersectionsTableKey ] )
			euclidean.addXIntersectionIndexesFromXIntersections( 1, xIntersectionIndexList, aboveIntersectionsTable[ aboveIntersectionsTableKey ] )
			supportLayer.xIntersectionsTable2[ supportIntersectionsTableKey ] = euclidean.getJoinOfXIntersectionIndexes( xIntersectionIndexList )
	...
	def addSupportLayerTemperature( self, endpoints, z ):
		...
		travelfeedRateMinute=60*self.speedclass.travelFeedRatePerSecond.value
		for path in paths:
			self.distanceFeedRate.addGcodeFromFeedRateThreadZ( self.feedRateMinute, travelfeedRateMinute, path, z )
	...
	def getStepsUntilEnd( self, begin, end, stepSize ):
		...
		while step < end:
			...
			step += stepSize
		return steps
	def getSupportEndpoints3( self ):
		"BFB CODE will be support at 1st layer after Raft"
		if len( self.supportLayers ) <= 0:
			return []
		supportSegmentTable = self.supportLayers[ 0 ].supportSegmentTable
		endpoints = euclidean.getEndpointsFromSegmentTable( supportSegmentTable )
		return endpoints
	def getSupportEndpoints2( self ):
		"BFB CODE(will be support on next layer)"
		if len( self.supportLayers ) <= (self.layerIndex+1):
			return []
		
		supportSegmentTable = self.supportLayers[ self.layerIndex+1 ].supportSegmentTable
		endpoints = euclidean.getEndpointsFromSegmentTable( supportSegmentTable )
		return endpoints
	def getSupportEndpoints( self ):
		...
		layer=self.layerIndex+1
		layer2= self.layerIndex
		if len( self.supportLayers )>layer:
		       layer2=layer
		"Get Model points----------------------------------------------------------------"
		if len(self.boundaryLayers)>(self.layerIndex+1):
			aboveLayer = self.boundaryLayers[ self.layerIndex]
			aboveLoops = aboveLayer.loops
			numberOfSubSteps = 4
			subStepSize = self.interfaceStep / float( numberOfSubSteps )
			aboveIntersectionsTable = {}
			...
			aboveLayer = self.boundaryLayers[ self.layerIndex + 1 ]
			aboveLoops2 = aboveLayer.loops
			numberOfSubSteps = 4
			subStepSize = self.interfaceStep / float( numberOfSubSteps )
			aboveIntersectionsTable2 = {}
			euclidean.addXIntersectionsFromLoopsForTable( aboveLoops2, aboveIntersectionsTable2, subStepSize )
			
			
		"Get Support points--------------------------------------------------------------"       
		supportSegmentTable = self.supportLayers[ self.layerIndex ].supportSegmentTable
		supportSegmentTable2 = self.supportLayers[ layer2 ].supportSegmentTable
		endpoints = euclidean.getEndpointsFromSegmentTable( supportSegmentTable )
		endpoints2 = euclidean.getEndpointsFromSegmentTable( supportSegmentTable2)
		"Comparison perimeters------------------------------------------------------------"
		cross=False
		if len(endpoints)>0 and len(endpoints2)>0:
			pointx=endpoints[0].point.real
			pointy=endpoints[0].point.imag
			point2x=endpoints[-1].point.real
			point2y=endpoints[-1].point.imag
			pointx=abs(point2x-pointx)
			pointy=abs(point2y-pointy)
			firstperimeter=(pointx+pointy)*2
		
			pointx=endpoints2[0].point.real
			pointy=endpoints2[0].point.imag
			point2x=endpoints2[-1].point.real
			point2y=endpoints2[-1].point.imag
			pointx=abs(point2x-pointx)
			pointy=abs(point2y-pointy)
			secondperimeter=(pointx+pointy)*2


			if abs(firstperimeter-secondperimeter)>0.5:
				cross=True
		if len(self.boundaryLayers)>(self.layerIndex+1):
			if len(aboveIntersectionsTable)>0 and len(aboveIntersectionsTable2)>0:
				key=aboveIntersectionsTable.keys()[0]
				points=aboveIntersectionsTable[key]
				if len(points)>2:
					 modelx=points[0]
					 model2x=points[-2]
					 width1=abs(modelx-model2x)
				else:
					modelx=points[0]
					model2x=points[-1]
					width1=abs(modelx-model2x)
				key2=aboveIntersectionsTable2.keys()[0]        
				points=aboveIntersectionsTable2[key2]
				if len(points)>2:
					 modelx=points[0]
					 model2x=points[-2]
					 width2=abs(modelx-model2x)
				else:
					modelx=points[0]
					model2x=points[-1]
					width2=abs(modelx-model2x)
		...
		"Good Support Layer before Model layer"
		if len( self.supportLayers )<=layer or len(self.supportLayers[ layer2 ].supportSegmentTable)<1 or cross == True:
			crossEndpoints = []
			crossHatchPointLineTable = {}
			for endpoint in endpoints:
				segmentBeginXStep = int( math.ceil( min( endpoint.point.real, endpoint.otherEndpoint.point.real ) / 1 ) )
				segmentEndXStep = int( math.ceil( max( endpoint.point.real, endpoint.otherEndpoint.point.real ) / 1 ) )
		...
		if self.layerIndex % 2 == 0 or not self.repository.supportCrossHatch.value or len( self.supportLayers )<=layer and len(self.supportLayers[ layer2 ].supportSegmentTable)<1 or cross == True:
	...
	def supportstart(self):
		"BFB CODE INITIALIZE before support"
		self.bfbclass = bfb.BfbRepository()
		self.bfbclass = settings.getReadRepository( self.bfbclass )
		self.firstHeadReverseRev = chan(self.bfbclass.firstHeadReverseRev.value, 6400)
		self.firstHeadReverseSpeed = chan(self.bfbclass.firstHeadReverseSpeed.value, 10)
		...
		"Use second Extruder for support"
		if self.bfbclass.useSupport.value == True:
			self.distanceFeedRate.LS2 = True
			if self.version == "x1":
				self.distanceFeedRate.addLine( 'M228' )
				self.distanceFeedRate.addLine( 'M542' )
				self.distanceFeedRate.addLine( 'M561 P'+str(self.firstHeadReverseRev)+' S'+str(self.firstHeadReverseSpeed) )
				self.distanceFeedRate.addLine( 'M543' )
				self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.firstStandbyT.value) )
				self.distanceFeedRate.addLine( 'G92 X'+str(self.bfbclass.secondHeadOffsetX.value)+' Y'+str(self.bfbclass.secondHeadOffsetY.value)+' F6000' )
				self.distanceFeedRate.addLine( 'M542' )
				self.distanceFeedRate.addLine( 'M107' )
				self.distanceFeedRate.addLine( 'M204 S'+str(self.bfbclass.SupportT.value) )
			...
			"Use first Extruder for support"
			self.distanceFeedRate.addLine( 'M107' )
			self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.SupportT.value) )
			if self.bfbclass.UseFanEx1.value== True:
				"Turn ON FAN"
	...
	def supportend(self):
		"BFB CODE INITIALIZE after support"
		self.bfbclass = bfb.BfbRepository()
		self.bfbclass = settings.getReadRepository( self.bfbclass )
		self.secondHeadReverseRev = chan(self.bfbclass.secondHeadReverseRev.value, 6400)
		self.secondHeadReverseSpeed = chan(self.bfbclass.secondHeadReverseSpeed.value, 10)
		self.firstHeadPrimeRev = chan(self.bfbclass.firstHeadPrimeRev.value, 6400)
		self.firsHeadPrimeSpeed = chan(self.bfbclass.firsHeadPrimeSpeed.value, 10)
		self.firstHeadPrintRev = chan(self.bfbclass.firstHeadPrintRev.value, 6400)
		self.firstHeadPrintPrime = chan(self.bfbclass.firstHeadPrintPrime.value, 6400)
		...
		if self.bfbclass.useSupport.value == True:
			if self.version == "x1":
				self.distanceFeedRate.addLine( 'M228' )
				self.distanceFeedRate.addLine( 'M542' )
				self.distanceFeedRate.addLine( 'M562 P'+str(self.secondHeadReverseRev)+' S'+str(self.secondHeadReverseSpeed) )
				self.distanceFeedRate.addLine( 'M543' )
				self.distanceFeedRate.addLine( 'M204 S'+str(self.bfbclass.secondStandbyT.value) )
				self.distanceFeedRate.addLine( 'G90' )
				self.distanceFeedRate.addLine( 'M542' )
			...
			"Use just one Extruder for support"
			self.distanceFeedRate.addLine( 'M107' )
			self.distanceFeedRate.addLine( 'M104 S'+str(self.bfbclass.firstWorkT.value) )
			if self.bfbclass.UseFanEx1.value== True:
				"Turn ON FAN"
	...
	def parseLine( self, line ):
		...
		if firstWord == 'G1':
			...
			if self.layerStarted and self.addLineLayerStart:
				...
				self.distanceFeedRate.addLine( '(</layer>)' )
			self.layerStarted = False
			"BFB CODE Raft , finish M101-->M201"
			self.distanceFeedRate.LS = False
			...
			line = ''
			self.endp = endpoints
			self.endp2 = self.getSupportEndpoints2()
			...
			if len( endpoints ) > 0:
				"BFB CODE using support material"
				"Support start in start of layer"
				if self.swap ==1 and self.endlayer == True :
					self.endlayer = False
					
					"CORRECT INITIALIZATION"
					if self.bfbclass.useRaft.value == False and self.bfbclass.useSupport.value == True:
						self.start = 1
					if self.bfbclass.useRaft.value == True and self.bfbclass.useSupport.value == False:
						self.start = 1
					...
					if self.start == 1:
						self.end = 1
						self.supportstart()			
				"Support end in start of layer"
				if self.swap ==1:
					self.swap = 2
					"Writing support g-code"
			...
			if len( self.endp ) > 0:
				"Support start in end of layer"
				if self.swap == 2 and self.endlayer == True:
					self.swap = 1
					self.supportstart()
					self.start = 0
					"Writing support g-code"
					self.addSupportLayerTemperature( self.endp, self.lz )
					"Check is next layer not using support"
					if self.endp2 ==[] and self.end == 1 :
						self.end = 0
						self.supportend()
						self.start = 1
	...
	def setBoundaryLayers( self ):
		...
		for supportLayerIndex in xrange( len( self.supportLayers ) - 2, - 1, - 1 ):
			...
			xIntersectionsTable2 = self.supportLayers[ supportLayerIndex ].xIntersectionsTable2
			aboveXIntersectionsTable2 = self.supportLayers[ supportLayerIndex + 1 ].xIntersectionsTable2
			euclidean.joinXIntersectionsTables( aboveXIntersectionsTable2, xIntersectionsTable2 )
		for supportLayerIndex in xrange( len( self.supportLayers ) ):
			...
			self.extendXIntersections( supportLayer.supportLoops, self.raftOutsetRadius, supportLayer.xIntersectionsTable2 )
		for supportLayer in self.supportLayers:
			euclidean.subtractXIntersectionsTable( supportLayer.xIntersectionsTable, supportLayer.fillXIntersectionsTable )
...
class SupportLayer:
	...
	def __init__( self, supportLoops ):
		...
		self.supportSegmentTable2 = {}
		self.xIntersectionsTable = {}
		self.xIntersectionsTable2 = {}
