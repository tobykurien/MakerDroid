...
class DistanceFeedRate:
	...
	def __init__( self ):
		...
		self.LS = False
		self.LS2 = False
	"BBF CODE bfb Raft gcode writing"
	def BfBaddGcodeFromFeedRateThreadZ( self, feedRateMinute, travelfeedRateMinute, thread, z, Thickness ):
		"Add a thread to the output."
		if len( thread ) > 0:
			self.addGcodeMovementZWithFeedRate( travelfeedRateMinute, thread[ 0 ], z+Thickness+0.05 )
		else:
			print( "zero length vertex positions array which was skipped over, this should never happen" )
		
		"BFB CODE"
		self.addGcodeMovementZWithFeedRate( 3000.0, thread[0], -0.5 )
		if self.LS or self.LS2:
			self.addLine( "M201" )
		else:
			self.addLine( "M101" ) # Turn extruder on.
		self.addGcodeMovementZWithFeedRate( 1000.0, thread[0], z+Thickness+0.05 )
		self.addLine( "M103" ) # Turn extruder off.
		
		
		

	def addGcodeFromFeedRateThreadZ( self, feedRateMinute, travelfeedRateMinute, thread, z ):
		"Add a thread to the output."
		if len( thread ) > 0:
	...
	def addGcodeFromThreadZ( self, thread, z ):
		...
		"BFB CODE"
		if self.LS or self.LS2:
			self.addLine( "M201" ) # Turn extruder on.
		else:
			self.addLine( "M101" ) # Turn extruder on.
		for point in thread[ 1 : ]:
