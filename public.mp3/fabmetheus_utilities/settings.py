...
def addListsToRepository( fileNameHelp, profileDirectory, repository ):
	...
	repository.title = 'BfB '+repository.capitalizedName + ' Settings'
...
def getfirmware():
        "BFB CODE get version of firmware"
        path=getSettingsDirectoryPath('');
        f = open(path+"/firmware.csv")
...
def getSettingsDirectoryPath( subfolder = '' ):
	...
	"BFB code Get path of settings from registry"
	#key=_winreg.OpenKey(_winreg.HKEY_CURRENT_USER, r"Software\bfb")
	#name, value, type =_winreg.EnumValue(key, 0)
	#dir_of_settings=repr(value)
	#dir_of_settings=dir_of_settings.replace('u','')
	#dir_of_settings=dir_of_settings.replace("'","")
	import posix
	settingsDirectory = posix.environ['SETTINGS_DIRECTORY']
	#settingsDirectory =  SETTINGS_DIRECTORY #"/mnt/sdcard/Skeinforge/settings"
	if subfolder == '':
